...
knitr::opts_chunk$set(echo = TRUE) #This line of code tells the knitr package to display code from chunks.
library(tidyverse) #This line of code will load the tidyverse package which will allow us to use specific functions. It's like telling our computer what cookbook we need in order to find a specific recipie. 

library(openintro)
```

Before starting any lab, be sure to change the author and date in the yaml header to be your name and today's date. Note that the further along we get in the semester, the less explanatory information will be written out in labs. For example, the reminder to change the author and date will not be in every lab but you're still expected to do so.
