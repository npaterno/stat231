...
For this lab we will be trying to predict the difference between the starting price and final price (not including shipping) for a Mario Kart game auctioned on ebay. Let's take a look at our dataset. 
```{r}
glimpse(mariokart)
```
...
Now that our model is built, let's make some predictions. To do this, we'll need to write a function that takes in a model and value and returns a prediction. 

```{r}
model_prediction <- function(model, value){ #This line defines our function.
  return(model$coefficients[1]+value*model$coefficients[2]) #This line calculates the prediction by evaluating the linear function at the supplied value and returns the result. 
}
```
...
Interpolation: 
```{r}
model_prediction(price_model, 15)
```
...
Extrapolation: 
```{r}
model_prediction(price_model, 100)
```
